...
class TipRepository {
  ...
  // Get personalized tip data
  Future<Map<String, dynamic>> getPersonalizedTip({
    bool useCache = true,
  }) async {
    try {
  ...
  Future<List<Map<String, dynamic>>> _getRecentWorkouts(String userId) async {
    try {
      ...
      final workoutDocs = await _firestore
          .collection('users')
          .doc(userId)
  ...
  Future<List<Map<String, dynamic>>> _getTodaysFoodLogs(String userId) async {
    try {
  ...
  Future<Map<String, dynamic>> _getCalorieData(String userId) async {
    try {
      // Get user macros
      final macroDoc = await _firestore
          .collection('users')
          .doc(userId)
  ...
  List<Map<String, dynamic>> _sanitizeFirebaseData(List<Map<String, dynamic>> data) {
  ...
  // Save tip to cache
  Future<void> _cacheTip(Map<String, dynamic> tipData) async {
    try {
  ...
  Future<Map<String, dynamic>?> _getCachedTip() async {
    try {
      ...
      if (now.difference(cachedTime) < CACHE_DURATION) {
        print('Found valid cached tip from ${now.difference(cachedTime).inMinutes} minutes ago');
        return cachedData['tip'];
      } else {
        print('Found expired cached tip from ${now.difference(cachedTime).inHours} hours ago');
      }
  ...
  Map<String, dynamic> _getFallbackTip() {
    final fallbackTips = [
      {
